...
export default function LegalAuditVault({ 
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto w-full p-4 lg:p-6 bg-transparent text-slate-900 gap-5 no-scrollbar">
      ...
      <section className="relative flex flex-col flex-shrink-0 min-h-fit rounded-2xl p-5 bg-white border border-slate-200 shadow-xs">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          <div className="flex items-start gap-4 max-w-4xl">
            <div className="relative flex-shrink-0 flex items-center justify-center w-11 h-11 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 shadow-xs">
          ...
          <div className="flex flex-wrap items-center gap-2 flex-shrink-0">
            <button
              ...
              disabled={verifying}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-xs transition-all cursor-pointer active:scale-95 disabled:opacity-50"
            >
      ...
      <section className="relative flex flex-col flex-shrink-0 min-h-fit rounded-2xl bg-white p-5 border border-emerald-200 shadow-xs">
        <div className="relative flex flex-col gap-4 z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-emerald-50 flex items-center justify-center flex-shrink-0 text-emerald-700 border border-emerald-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 pt-1">
            <div className="p-3 rounded-xl bg-slate-50 flex flex-col gap-1 border border-slate-200">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono uppercase text-slate-500 font-bold tracking-wider">Current Tip Block</span>
                <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-sky-50 text-sky-700 border border-sky-200 font-bold">
                  BLOCK #{auditLogs.length}
                </span>
              </div>
              <div className="flex items-center justify-between gap-2 mt-1">
                <span className="text-xs text-sky-700 font-mono truncate font-semibold" title={String(latestTipHash || '')}>
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-sky-700 transition-colors flex-shrink-0 cursor-pointer"
            ...
            <div className="p-3 rounded-xl bg-slate-50 flex flex-col gap-1 border border-slate-200">
              <span className="text-[10px] font-mono uppercase text-slate-500 font-bold tracking-wider">BSA Certificate Registry</span>
            ...
            <div className="p-3 rounded-xl bg-slate-50 flex flex-col gap-1 border border-slate-200">
              <span className="text-[10px] font-mono uppercase text-slate-500 font-bold tracking-wider">Attestation Timestamp</span>
              <div className="flex items-center gap-1.5 mt-1">
                <span className="material-symbols-outlined text-sky-600 text-[15px]">schedule</span>
            ...
            <div className="p-3 rounded-xl bg-slate-50 flex flex-col gap-1 border border-slate-200">
              <span className="text-[10px] font-mono uppercase text-slate-500 font-bold tracking-wider">Certifying Officer</span>
      ...
      <section className="flex flex-col flex-shrink-0 min-h-fit gap-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-sky-600 text-[18px]">account_balance</span>
            <h2 className="text-xs font-mono font-bold text-slate-900 tracking-wider uppercase">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {/* Finding 1 */}
          <div className="flex flex-col justify-between rounded-2xl bg-white p-4.5 border border-slate-200 hover:border-sky-300 transition-all shadow-xs">
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full font-mono text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-200">
                  MASTERMIND CUT-OUT
                </span>
              </div>
          ...
          {/* Finding 2 */}
          <div className="flex flex-col justify-between rounded-2xl bg-white p-4.5 border border-slate-200 hover:border-amber-300 transition-all shadow-xs">
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[10px] font-mono text-slate-500 uppercase font-bold tracking-wider">
          ...
          <div className="flex flex-col justify-between rounded-2xl bg-white p-4.5 border border-slate-200 hover:border-sky-300 transition-all shadow-xs">
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full font-mono text-[10px] font-bold bg-sky-50 text-sky-700 border border-sky-200">
                  SEC 66D IT ACT
                </span>
              </div>
              <h3 className="font-display text-sm font-bold text-slate-900 mt-1">
      ...
      <section className="p-5 rounded-2xl bg-white border border-slate-200 flex flex-col flex-shrink-0 min-h-fit gap-4 shadow-xs">
          ...
          {auditLogs.map((log, idx) => {
            ...
            return (
              <div
                ...
                className="p-4 rounded-xl bg-slate-50 border border-slate-200 flex flex-col gap-2.5 hover:border-sky-300 transition-all shadow-xs"
              >
                <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-md bg-sky-50 text-sky-700 border border-sky-200 font-bold font-mono text-[10px]">
                      BLOCK #{idx + 1}
                    </span>
                    <span className="text-slate-900 font-bold">{actionText}</span>
                    <span className="text-slate-300">•</span>
                    <span className="text-slate-500 font-mono text-[11px]">{log.timestamp}</span>
                  </div>
                  <span className="text-slate-500 font-mono text-[11px]">Investigator: <span className="text-slate-900 font-semibold">{investigatorText}</span></span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 pt-2 border-t border-slate-200 font-mono text-[10px]">
